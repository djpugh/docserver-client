...
``docsclient`` is a volunteer maintained open source project and we welcome contributions of all forms. The sections
below will help you get started with development, testing, and documentation. We’re pleased that you are interested in
working on ``docsclient``. This document is meant to get you setup to work on ``docsclient`` and to act as a guide and reference
to the development setup. If you face any issues during this process, please
`open an issue <https://github.com/djpugh/docserver-client/issues/new?title=Trouble+with+development+environment>`_ about it on
the issue tracker.

Setup
~~~~~

``docsclient`` is an client for |docserver| :github:`djpugh/docserver` to provide upload methods
written in Python. To work on it, you'll need:

- **Source code**: available on `GitHub <https://github.com/djpugh/docserver-client>`_. You can use ``git`` to clone the
    repository:

  .. code-block:: console

      git clone https://github.com/djpugh/docserver-client
      cd docsclient
...
- **Docserver instance**: We recommend using the |docserver-docker| :docker:`djpugh/docserver`.


Running tests
~~~~~~~~~~~~~

``docsclient`` tests are written using the :pypi:`pytest` test framework. :pypi:`tox` is used to automate the setup
and execution of ``docsclient`` tests.
...
``docsclient`` uses :pypi:`flake8` and extensions for managing linting of the codebase. ``flake8`` performs various checks on all
files in ``docsclient`` and uses tools that help follow a consistent code style within the codebase. To use linters locally,
run:
...
``docsclient`` documentation is built using :pypi:`Sphinx`. The documentation is written in reStructuredText. To build it
locally, run:
...
have legal permission to distribute any code you contribute to ``docsclient`` and it must be available under the MIT
License. Provide tests that cover your changes and run the tests locally first. ``docsclient``
